...
/// Total half-width of a bar in world units (along the camera right vector).
const BAR_HALF_WIDTH: f32 = 3.5;
/// Depth of a bar perpendicular to the camera right vector (appears as bar height).
...
/// Derive the camera right vector (projected onto the XZ plane) from the camera's
/// forward direction.  Bars aligned along this vector always face the player.
///
/// Formula: `(0,1,0) × forward` projected to XZ = `(forward.z, 0, -forward.x)`.
pub fn camera_right_xz(cam_forward: Vec3) -> Vec3 {
    Vec3::new(cam_forward.z, 0.0, -cam_forward.x).normalize_or_zero()
}

/// Append a camera-aligned quad to `vertices`/`indices`.
///
/// The quad lies flat (normal = Y-up), with its long axis along `cam_right` and
/// its short axis along the perpendicular XZ direction.
///
/// `bar_start` and `bar_end` are signed distances from the centre along `cam_right`.
fn add_rect(
    cx: f32,
    cy: f32,
    cz: f32,
    ...
    // Perpendicular direction in the XZ plane (90° rotation of cam_right).
    let perp = Vec3::new(-cam_right.z, 0.0, cam_right.x);
    let normal = [0.0_f32, 1.0, 0.0];
    let centre = Vec3::new(cx, cy, cz);

    let p0 = centre + perp * (-half_d) + cam_right * bar_start;
    let p1 = centre + perp *   half_d  + cam_right * bar_start;
    let p2 = centre + perp *   half_d  + cam_right * bar_end;
    let p3 = centre + perp * (-half_d) + cam_right * bar_end;

    vertices.push(MeshVertexAttributes { position: p0.to_array(), normal, uv: [0.0, 0.0], color: [255, 255, 255, 255] });
    vertices.push(MeshVertexAttributes { position: p1.to_array(), normal, uv: [1.0, 0.0], color: [255, 255, 255, 255] });
    vertices.push(MeshVertexAttributes { position: p2.to_array(), normal, uv: [1.0, 1.0], color: [255, 255, 255, 255] });
    vertices.push(MeshVertexAttributes { position: p3.to_array(), normal, uv: [0.0, 1.0], color: [255, 255, 255, 255] });
...
/// `cam_forward` is the camera's current forward direction (world space).
pub fn update_health_bar_meshes(
    ...
    let cam_right = camera_right_xz(cam_forward);
    ...
    for (entity, (health, pos)) in world.query::<(&Health, &Position)>().iter() {
        ...
        add_rect(cx, cy, cz, cam_right, -BAR_HALF_WIDTH, BAR_HALF_WIDTH, &mut bg_v, &mut bg_i);

        let fill_end = -BAR_HALF_WIDTH + frac * 2.0 * BAR_HALF_WIDTH;
        let fy = cy + FILL_Y_LIFT;
        if frac > 0.6 {
            add_rect(cx, fy, cz, cam_right, -BAR_HALF_WIDTH, fill_end, &mut green_v, &mut green_i);
        } else if frac > 0.3 {
            add_rect(cx, fy, cz, cam_right, -BAR_HALF_WIDTH, fill_end, &mut yellow_v, &mut yellow_i);
        } else {
            add_rect(cx, fy, cz, cam_right, -BAR_HALF_WIDTH, fill_end, &mut red_v, &mut red_i);
        }
    }
    ...
    if let Some(entity) = selected {
        if let (Ok(metab), Ok(pos)) = (
            ...
            if world.get::<&LeakyNeuron>(entity).is_ok() {
                ...
                let fill_end = -BAR_HALF_WIDTH + frac * 2.0 * BAR_HALF_WIDTH;
                add_rect(cx, cy + FILL_Y_LIFT, cz, cam_right, -BAR_HALF_WIDTH, fill_end, &mut fill_v, &mut fill_i);
            }
        }
    }
