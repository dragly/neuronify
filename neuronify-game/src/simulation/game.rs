...
use neuronify_core::{
    Compartment, CompartmentCurrent, Connection, GeneratorDynamics, LeakCurrent, LeakyDynamics,
    LeakyNeuron, NeuronType, Position, RegularSpikeGenerator, SpatialDynamics, FHN_CDT, LIF_DT,
    NODE_RADIUS, PHYSICS_DT,
};
...
/// Check if a position is within build range of any owned blood vessel.
pub fn is_within_vessel_range(world: &hecs::World, position: Vec3, player: PlayerId) -> bool {
    for (_, (p, vessel, o)) in world
        .query::<(&Position, &BloodVessel, &Ownership)>()
        .iter()
    {
        if o.player == player {
            let dist =
                Vec3::new(p.position.x - position.x, 0.0, p.position.z - position.z).length();
            if dist < vessel.supply_radius {
                return true;
            }
        }
    }
...
/// Check if a neuron can be built at position: requires glial coverage AND vessel range.
pub fn is_within_build_range(world: &hecs::World, position: Vec3, player: PlayerId) -> bool {
    is_within_vessel_range(world, position, player)
        && is_within_glial_range(world, position, player)
}
...
/// Glial cells distribute stored ATP to nearby owned neurons.
pub fn glial_distribute_atp(world: &mut hecs::World, dt: f64) {
    let glials: Vec<(Entity, Vec3, f32, f64, Option<PlayerId>)> = world
        .query::<(&GlialCell, &Position)>()
        .iter()
        .map(|(e, (g, p))| {
            let player = world.get::<&Ownership>(e).map(|o| o.player).ok();
            (e, p.position, g.distribute_radius, g.atp_stored, player)
        })
        .collect();

    for (glial_entity, gpos, distribute_radius, atp_stored, glial_player) in &glials {
        if *atp_stored <= 0.0 {
            continue;
        }

        let nearby_neurons: Vec<Entity> = world
            .query::<(&LeakyNeuron, &MetabolicState, &Position)>()
            .iter()
            .filter(|(_, (_, _, p))| {
                let dist = Vec3::new(p.position.x - gpos.x, 0.0, p.position.z - gpos.z).length();
                dist < *distribute_radius
            })
            .filter(|(e, _)| {
                let neuron_player = world.get::<&Ownership>(*e).map(|o| o.player).ok();
                neuron_player == *glial_player
            })
            .map(|(e, _)| e)
            .collect();

        if nearby_neurons.is_empty() {
            continue;
        }

        let supply_rate = RESOURCE_FLOW_RATE;
        let max_distribution = (supply_rate * dt).min(*atp_stored);
        let per_neuron = max_distribution / nearby_neurons.len() as f64;

        let mut total_distributed = 0.0;
        for neuron_entity in &nearby_neurons {
            if let Ok(mut metab) = world.get::<&mut MetabolicState>(*neuron_entity) {
                let room = metab.max_energy - metab.energy;
                let actual = per_neuron.min(room);
                metab.energy += actual;
        ...
        if total_distributed > 0.0 {
            if let Ok(mut glial) = world.get::<&mut GlialCell>(*glial_entity) {
                glial.atp_stored = (glial.atp_stored - total_distributed).max(0.0);
            }
        }
    }
}

/// Check if a position is within glial distribute_radius of any owned glial cell.
pub fn is_within_glial_range(world: &hecs::World, position: Vec3, player: PlayerId) -> bool {
    for (_, (p, glial, o)) in world.query::<(&Position, &GlialCell, &Ownership)>().iter() {
        if o.player == player {
            let dist =
                Vec3::new(p.position.x - position.x, 0.0, p.position.z - position.z).length();
            if dist < glial.distribute_radius {
                return true;
            }
        }
    }
...
/// Apply sensor neuron detection — inject current based on sensor type.
pub fn apply_sensors(world: &mut hecs::World) {
    let sensor_entities: Vec<(Entity, SensorType, f32, f64)> = world
        .query::<(&SensorNeuron, &Position)>()
        .iter()
        .map(|(e, (s, _))| (e, s.sensor_type.clone(), s.sensitivity, s.gain))
        .collect();

    for (entity, sensor_type, sensitivity, gain) in sensor_entities {
        let pos = match world.get::<&Position>(entity) {
            Ok(p) => p.position,
            Err(_) => continue,
        };
        let my_player = world.get::<&Ownership>(entity).map(|o| o.player).ok();

        let current = match sensor_type {
            SensorType::Activity => {
                // Count recently-fired neurons within sensitivity radius
                let count: usize = world
                    .query::<(&LeakyDynamics, &Position)>()
                    .iter()
                    .filter(|(e, _)| *e != entity)
                    .filter(|(_, (d, p))| {
                        d.time_since_fire < 0.01
                            && Vec3::new(p.position.x - pos.x, 0.0, p.position.z - pos.z).length()
                                < sensitivity
                    })
                    .count();
                gain * count as f64
            ...
            SensorType::Chemical => {
                // Detect SubstrateZone proximity — inject based on intensity
                let zones: Vec<(Vec3, f32)> = world
                    .query::<(&SubstrateZone, &Position)>()
                    .iter()
                    .map(|(_, (z, p))| (p.position, z.radius))
                    .collect();
                let mut total_intensity = 0.0f64;
                for (zpos, zradius) in &zones {
                    let dist = Vec3::new(pos.x - zpos.x, 0.0, pos.z - zpos.z).length();
                    if dist < *zradius {
                        total_intensity += (1.0 - dist / *zradius) as f64;
                    }
                }
                gain * total_intensity
            }
            SensorType::Touch => {
                // Detect other-player entities within sensitivity radius
                let count: usize = world
                    .query::<(&Position, &Ownership)>()
                    .with::<&LeakyNeuron>()
                    .iter()
                    .filter(|(_, (p, o))| {
                        Some(o.player) != my_player
                            && Vec3::new(p.position.x - pos.x, 0.0, p.position.z - pos.z).length()
                                < sensitivity
                    })
                    .count();
...
pub fn metabolic_drain(world: &mut hecs::World, dt: f64) {
    ...
    // Count non-dendrite compartments owned by each neuron (axon segments cost energy,
    // but dendrites are intrinsic to the neuron and free to maintain)
...
/// Disable neurons at 0 energy (dormancy). They stop firing but remain structurally.
/// Re-enable when energy is restored via glial cells.
pub fn apply_dormancy(world: &mut hecs::World) {
    ...
    for entity in entities {
        ...
        let is_blocked = world
            .get::<&DepolarizationBlock>(entity)
            .map(|b| b.blocked)
            .unwrap_or(false);

        // Don't override depolarization block's control of enabled
        if !is_blocked {
            if let Ok(mut dynamics) = world.get::<&mut LeakyDynamics>(entity) {
...
pub fn update_ownership(world: &mut hecs::World) {
    ...
    // Blood vessels: owned by whichever player has an owned neuron within supply_radius
    let vessels: Vec<(Entity, Vec3, f32)> = world
        .query::<(&BloodVessel, &Position)>()
        .iter()
        .map(|(e, (v, p))| (e, p.position, v.supply_radius))
        .collect();
    let owned_neurons: Vec<(Vec3, PlayerId)> = world
        .query::<(&Position, &Ownership)>()
        .with::<&LeakyNeuron>()
        .iter()
        .map(|(_, (p, o))| (p.position, o.player))
        .collect();
    for (vessel_entity, vessel_pos, supply_radius) in vessels {
        let mut p1_near = false;
        let mut p2_near = false;
        for (npos, nplayer) in &owned_neurons {
            let dist = Vec3::new(npos.x - vessel_pos.x, 0.0, npos.z - vessel_pos.z).length();
            if dist < supply_radius {
                match nplayer {
                    PlayerId::Player1 => p1_near = true,
                    PlayerId::Player2 => p2_near = true,
                }
            }
        }
        match (p1_near, p2_near) {
            (true, false) => {
                let _ = world.insert_one(
                    vessel_entity,
                    Ownership {
            ...
            _ => {
                let _ = world.remove_one::<Ownership>(vessel_entity);
            }
        }
    }

    // Glial cells: owned by whichever player has an owned neuron within gather_radius
    let glial_cells: Vec<(Entity, Vec3, f32)> = world
        .query::<(&GlialCell, &Position)>()
        .iter()
        .map(|(e, (g, p))| (e, p.position, g.gather_radius))
        .collect();
    for (glial_entity, glial_pos, gather_radius) in glial_cells {
        let mut p1_near = false;
        let mut p2_near = false;
        for (npos, nplayer) in &owned_neurons {
            let dist = Vec3::new(npos.x - glial_pos.x, 0.0, npos.z - glial_pos.z).length();
            if dist < gather_radius {
                match nplayer {
                    PlayerId::Player1 => p1_near = true,
                    PlayerId::Player2 => p2_near = true,
        ...
        match (p1_near, p2_near) {
            ...
            _ => {
                let _ = world.remove_one::<Ownership>(glial_entity);
            }
        }
    }
}
...
pub fn setup_game(world: &mut hecs::World, dish: &PetriDish) {
    ...
    // Player 1 origin - left side (immovable pacemaker)
    world.spawn((
        Position {
    ...
    // Player 2 origin - right side (AI, immovable pacemaker)
    world.spawn((
        Position {
            position: Vec3::new(offset, 0.0, 0.0),
        },
    ...
    // Blood vessels - scattered across the dish
    // Each player starts with one vessel near their origin; neutral vessels are contested
    let vessel_positions = [
        // Player starting vessels (will be claimed by proximity)
        Vec3::new(-offset * 0.6, 0.0, 0.0), // near P1
        Vec3::new(offset * 0.6, 0.0, 0.0),  // near P2
        // Neutral contested vessels
    ...
    for pos in vessel_positions {
        world.spawn((Position { position: pos }, BloodVessel::default()));
    }
...
pub fn run_headless_game(frames: usize, iterations_per_frame: u32) -> GameResult {
    ...
    let mut ai1 = crate::simulation::ai::AiState::new();
    let mut ai2 = crate::simulation::ai::AiState::new();
    let mut p1_economy = PlayerEconomy::default();
    let mut p2_economy = PlayerEconomy::default();
    ...
    for _frame in 0..frames {
        ...
        for _ in 0..iterations_per_frame {
            neuronify_core::lif_step(&mut world, lif_dt, time);
            time += lif_dt;
        }
        ...
        for _ in 0..iterations_per_frame {
            neuronify_core::fhn_step(&mut world, FHN_CDT, &recently_fired);
            neuronify_core::simulation::apply_spatial_forces(&mut world);
            neuronify_core::simulation::integrate_motion(&mut world, PHYSICS_DT);
        }
